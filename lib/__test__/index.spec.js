...
describe("mdLinks", () => {
  ...
  it('o teste deverá listar links sem valida-los', () => {
    return expect(mdLinks('meuarquivo.md', { validate: false })).resolves.toStrictEqual([
      {
        text: 'Markdown',
  ...
  it('o teste deverá listar links com validação', () => {
    return expect(mdLinks('meuarquivo.md', { validate: true })).resolves.toStrictEqual([
      {
        text: 'Markdown',
        href: 'https://pt.wikipedia.org/wiki/Markdown',
        status: 200
  ...
  it('testando com erro do fs', () => {
    const fs = require('fs');
    fs.readFile.mockImplementation( (file, option, cb) => cb( 'error', null) );
    return expect(mdLinks('meuarquivo.md', { validate: false })).rejects.toMatch('error');
  });
